...
<div style="display:flex;gap:3px;margin-bottom:24px;background:#f0ede8;padding:4px;border-radius:10px;width:fit-content">
    ...
    <button class="stab-btn" data-tab="passwords">
        ...
        Passwords
        @if($pendingCount > 0)
            <span style="background:var(--red);color:#fff;font-size:10px;font-weight:700;padding:1px 7px;border-radius:20px;margin-left:2px">{{ $pendingCount }}</span>
        @endif
    </button>
    <button class="stab-btn" data-tab="archived">
        ...
        Archived
        @if($archivedCount > 0)
            <span style="background:var(--orange);color:#fff;font-size:10px;font-weight:700;padding:1px 7px;border-radius:20px;margin-left:2px">{{ $archivedCount }}</span>
        @endif
    </button>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span style="font-family:monospace;font-size:12px;background:#f0ede8;padding:2px 8px;border-radius:5px">{{ $user->username }}</span></td>
                        <td><span class="badge {{ $user->role==='admin' ? 'badge-active' : ($user->role==='viewer' ? 'badge-inactive' : 'badge-pending') }}">{{ ucfirst($user->role) }}</span></td>
                        ...
                        <td>
                            <div class="flex gap-2">
                                ...
                                @if($user->id !== auth()->id())
                                <form method="POST" action="{{ route('settings.users.delete', $user) }}" style="display:inline"
...
<div id="stab-passwords" class="stab-pane" style="display:none">
    <div style="font-size:15px;font-weight:700;color:var(--text);margin-bottom:16px">
        ...
        @if($pendingCount > 0)
            <span style="background:var(--red);color:#fff;font-size:11px;font-weight:700;padding:2px 8px;border-radius:20px;margin-left:8px">{{ $pendingCount }} pending</span>
        @endif
    </div>
    <div class="card">
        <div class="table-wrap">
                        ...
                        <td>
                            ...
                            <div class="flex gap-2">
                                <button class="btn btn-success btn-sm s-approve-pw"
                                    data-url="{{ route('auth.approve-password', $req) }}"
                                    data-id="{{ $req->id }}">
                                ...
                                <button class="btn btn-sm s-reject-pw"
                                    ...
                                    data-url="{{ route('auth.reject-password', $req) }}"
                                    data-id="{{ $req->id }}">
...
<div id="stab-archived" class="stab-pane" style="display:none">
    <div class="flex items-center justify-between mb-4">
        <div>
            ...
            <div style="font-size:13px;color:var(--text2);margin-top:2px">{{ $archivedCount }} archived — all records preserved</div>
        </div>
    </div>
    <div class="card">
        <div class="table-wrap">
                    ...
                    <tr>
                        <td>
                            <div style="display:flex;align-items:center;gap:10px">
                                ...
                                <div>
                                    <div style="font-weight:600;font-size:13px;color:var(--text2)">{{ $emp->full_name }}</div>
                                    <div style="font-size:11.5px;color:#aaa">{{ $emp->employee_no }}</div>
                                </div>
                            </div>
                        </td>
                        <td style="color:var(--text2)">{{ $emp->position->title }}</td>
                        <td style="color:var(--text2)">{{ $emp->position->department->name }}</td>
                        <td style="color:var(--text2)">{{ $emp->hire_date->format('M d, Y') }}</td>
                        <td>
                            <div style="font-size:12.5px;color:var(--text2)">{{ $emp->deleted_at->format('M d, Y') }}</div>
                            <div style="font-size:11px;color:#bbb">{{ $emp->deleted_at->diffForHumans() }}</div>
                        </td>
                        <td>
                            <button class="btn btn-success btn-sm s-restore-emp"
                                data-url="{{ route('employees.restore', $emp->id) }}"
                                data-name="{{ addslashes($emp->full_name) }}">
...
<div id="stab-profile" class="stab-pane" style="display:none">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;max-width:800px">
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('settings.update-username') }}">
                    @csrf
                    <div class="form-group">
                        <label class="form-label">Current Username</label>
                        <input type="text" class="form-control" value="{{ auth()->user()->username }}" disabled style="background:#f0ede8;color:#999">
                    </div>
                    <div class="form-group">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form method="POST" action="{{ route('settings.change-password-direct') }}">
                    @csrf
                    <div class="form-group">
...
<div id="stab-deductions" class="stab-pane" style="display:none">
    ...
    <div class="card" style="max-width:680px">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('settings.deductions.update') }}" id="deductions-form">
                @csrf
                ...
                <div style="display:grid;grid-template-columns:1fr auto auto auto;gap:12px;align-items:center;padding:14px 0;border-bottom:1px solid var(--border)">
                    <div>
                        <div style="font-size:13.5px;font-weight:600;color:var(--text)">{{ $setting->label }}</div>
                        <div style="font-size:12px;color:var(--text2);margin-top:2px">{{ $setting->description }}</div>
                    </div>
                    <div style="display:flex;align-items:center;gap:6px">
                        <input type="number"
                               name="deductions[{{ $setting->key }}][value]"
                               value="{{ number_format($setting->value, 2) }}"
                    ...
                    <div>
                        <span class="badge {{ $setting->type === 'percentage' ? 'badge-blue' : 'badge-pending' }}" style="{{ $setting->type === 'percentage' ? 'background:var(--blue-bg);color:var(--blue)' : '' }}">
                            {{ ucfirst($setting->type) }}
                        </span>
                    </div>
                    <div>
                        <label style="display:flex;align-items:center;gap:6px;font-size:12.5px;color:var(--text2);cursor:pointer">
                            <input type="hidden" name="deductions[{{ $setting->key }}][is_active]" value="0">
                            <input type="checkbox"
                                   name="deductions[{{ $setting->key }}][is_active]"
                                   value="1"
        ...
        <div class="card">
            ...
            <div class="card-body">
                <table style="width:100%;font-size:13.5px">
                    ...
                    <tr><td style="color:var(--text2);padding:7px 0">Laravel</td><td>{{ app()->version() }}</td></tr>
                    <tr><td style="color:var(--text2);padding:7px 0">PHP</td><td>{{ PHP_VERSION }}</td></tr>
                    <tr><td style="color:var(--text2);padding:7px 0">Environment</td><td><span class="badge badge-active">{{ app()->environment() }}</span></td></tr>
                    <tr><td style="color:var(--text2);padding:7px 0">Timezone</td><td>{{ config('app.timezone') }}</td></tr>
                    <tr><td style="color:var(--text2);padding:7px 0">Active Employees</td><td>{{ \App\Models\Employee::where('status','active')->count() }}</td></tr>
                    <tr><td style="color:var(--text2);padding:7px 0">Total Users</td><td>{{ \App\Models\User::count() }}</td></tr>
                </table>
            </div>
        </div>
        <div class="card">
            ...
            <div class="card-body">
                <div style="background:#faf8f5;border-radius:8px;padding:14px;border:1px solid var(--border);font-size:13px;color:var(--text2);line-height:1.9">
                    ...
                    Status: <span class="badge {{ config('app.super_admin_key') ? 'badge-active' : 'badge-rejected' }}">
                        {{ config('app.super_admin_key') ? 'Configured' : 'Not Set' }}
                    </span>
                </div>
            </div>
        </div>
...
<div id="s-user-modal" style="display:none;position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,0.45);align-items:center;justify-content:center">
    <div style="background:var(--surface);border-radius:16px;width:100%;max-width:460px;margin:20px;box-shadow:0 20px 60px rgba(0,0,0,0.2);overflow:hidden">
        ...
        <div style="padding:22px 24px">
            <form method="POST" id="s-user-form" action="{{ route('settings.users.store') }}">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select name="employee_id" id="s-employee" class="form-control">
                        ...
                        @foreach(\App\Models\Employee::where('status','active')->orderBy('last_name')->get() as $emp)
...
{{-- ══ INLINE SCRIPT — works with AJAX nav ══ --}}
<style>
.stab-btn {
...
(function() {
    ...
    document.getElementById('btn-new-user').addEventListener('click', () => {
        document.getElementById('s-user-form').action = '{{ route("settings.users.store") }}';
