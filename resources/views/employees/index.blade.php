...
<div class="card mb-4" style="padding:16px 20px">
    <form method="GET" class="flex gap-2">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search name or employee no…" class="form-control" style="max-width:280px">
...
<div class="card">
    <div class="table-wrap">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td><span style="font-family:monospace;font-size:12px;color:#888">{{ $emp->employee_no }}</span></td>
                    <td>
                        <div style="display:flex;align-items:center;gap:10px">
                            ...
                            <div>
                                ...
                                <span class="text-muted">{{ $emp->position->title }}</span>
                            </div>
                        </div>
                    </td>
                    <td>{{ $emp->position->title }}</td>
                    <td>{{ $emp->position->department->name }}</td>
                    <td>{{ str_replace('_',' ', ucfirst($emp->employment_type)) }}</td>
                    <td>₱{{ number_format($emp->daily_rate,2) }}</td>
                    ...
                    <td>
                        <div class="flex gap-2">
                            ...
                            <a href="{{ route('employees.edit', $emp) }}" class="btn btn-accent btn-sm">Edit</a>
                            <button onclick="confirmArchive({{ $emp->id }}, '{{ addslashes($emp->full_name) }}', '{{ route('employees.archive', $emp) }}')"
                                    class="btn btn-sm"
    ...
    <div style="padding:0 20px">{{ $employees->withQueryString()->links() }}</div>
</div>
